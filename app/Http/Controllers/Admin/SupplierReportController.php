...
class SupplierReportController extends Controller
{
	...
	public function __construct(SupplierModel $supplier_model)
	{
		...
		$this->module_title       = "Suppliers Report";
	...
	/*
    | Function  : Get all the suppliers data
    | Author    : Deepak Arvind Salunke
    | Date      : 11/05/2018
    | Output    : Show all suppliers listing
    */
	...
	public function index()
	{
		...
		return view($this->module_view_folder.'.suppliers',$this->arr_view_data);
	} // end index
	...
	public function export_csv(Request $request)
	{
        ...
        if($obj_users)
        {
            ...
            foreach ($arr_user as $key => $user) 
            {
                ...
                $data['Admin Commission']   = isset($user['admin_commission'])? $user['admin_commission'] :'';
        ...
        return Excel::create('Suppliers', function($excel) use ($data) {

            // Set the title
            $excel->setTitle('Suppliers Reports');
            ...
            $excel->setDescription('Suppliers Reports');

            $excel->sheet('Suppliers', function($sheet) use ($data)
            {
