...
class CancellationReturnReportController extends Controller
{
	...
	public function __construct(OrdersModel $orders_model)
	{
		...
		$this->module_url_path    = $this->admin_url_path."/reports/cancellation-return";
		$this->module_title       = "Cancellation & Return Report";
		$this->module_view_folder = "admin.reports.cancellation_return";
    ...
    */
	...
	public function load_data(Request $request)
	{
		...
		$obj_reports = $obj_reports->select(['id', 'order_id','user_id', 'order_fname', 'order_lname', 'order_email', 'order_payment_method', 'status', 'comment', 'created_at'])
		...
		if($obj_reports)
		{
			...
			foreach ($build_result->data as $key => $data) 
			{
				if(isset($build_result->data) && sizeof($build_result->data)>0)
				{
					...
					$build_result->data[$key]->comment            = isset($data->comment) ? $data->comment : 'NA';
	...
	public function export_csv(Request $request)
	{
        ...
        if($obj_reports)
        {
            ...
            foreach ($arr_reports as $key => $reports) 
            {
                ...
                $data['Cancel Reason']         = isset($reports['comment'])? $reports['comment'] : 'NA';
        ...
        return Excel::create('Cancellation & Return', function($excel) use ($data) {

            // Set the title
            $excel->setTitle('Cancellation & Return Reports');
            ...
            $excel->setDescription('Cancellation & Return Reports');

            $excel->sheet('Cancellation & Return', function($sheet) use ($data)
            {
