...
class MetalColorController extends Controller
{
	...
	public function __construct(MetalColorModel $color_model)
	{
		$this->arr_data           = [];
		$this->MetalColorModel        = $color_model;
		$this->BaseModel        = $color_model;
		$this->admin_panel_slug   = config('app.project.admin_panel_slug');
		$this->admin_url_path     = url(config('app.project.admin_panel_slug'));
		$this->module_url_path    = $this->admin_url_path."/metal_colors";
		$this->module_title       = "Colors";
		$this->module_view_folder = "admin.metal_colors";
	...
	public function store(Request $request)
	{
		...
		$metal_color = $request->input('metal_color', null);
		$slug       = str_slug($metal_color);

		$dose_exist = $this->BaseModel->where('metal_color', '=', $metal_color)->count();
	...
	public function edit($enc_id)
	{
		...
		$obj_color = $this->BaseModel->where('id',$id)->select('metal_color')->first();
	...
	public function update(Request $request, $enc_id)
	{
		...
		if($obj_color->metal_color == $metal_color)
		{
		...
		$dose_exist = $this->BaseModel->where('metal_color', '=', $metal_color)->where('id', '!=', $id)->count();
	...
	public function load_data(Request $request)
	{
		...
		{
			$obj_colors = $obj_colors->where('metal_color', 'LIKE', "%".$arr_search_column['q_colorname']."%");	
		}
		...
		$obj_colors = $obj_colors->select(['id', 'metal_color', 'status', 'created_at']);
		...
		if($obj_colors)
		{
			...
			foreach ($build_result->data as $key => $data) 
			{
				...
				if(isset($build_result->data) && sizeof($build_result->data)>0)
				{
					...
					$metal_color = isset($data->metal_color)? $data->metal_color :'';
					$status     = isset($data->status)? $data->status :'';
					$created_at = isset($data->created_at)? get_formated_created_date($data->created_at) :'';

					$build_result->data[$key]->id          = $id;				
					$build_result->data[$key]->metal_color          = $metal_color;				
