...
class BulkUploadController extends Controller
{
		...
		)
	{
		...
		$this->module_url_path               = url('/admin/bulk-upload');
		$this->module_view_folder            = "admin.bulk_upload";
	...
	public function upload_products(Request $request)
	{
		...
		if($results)
		{
			...
			foreach ($results as $key => $product) 
			{
				...
				$upload_product['added_by_user_type'] = '1';
				$upload_product['added_by_user_id']   = '0';
				...
				$product_description = isset($product['discription'])? $product['discription'] :'';

				if($product_description=='')
				{
					$this->arr_view_data['line'] = $key+1;
					$this->arr_view_data['status'] = 'error';
					$this->arr_view_data['msg']  = 'Please enter product discription';
					return back()->with('data', $this->arr_view_data);
				}
				...
				$delivery_date = isset($product['dilivary_date'])? $product['dilivary_date']:'';
...
// -----------------------------------------------------------------------------------------
				$occations =isset($product['occations'])? $product['occations']:'';

				$occations_detail = $this->ExcelService->get_occations_detail($occations);

				if($occations_detail['status'])
				{
					unset($occations_detail['status']);
					$upload_product_occations = $occations_detail;	
				}
				else
				{
					$this->arr_view_data['line'] = $key+1;
					$this->arr_view_data['status'] = 'error';
					$this->arr_view_data['msg']  = $occations_detail['msg'];
					return back()->with('data', $this->arr_view_data);
				}
...
// -----------------------------------------------------------------------------------------
				...
				isset($upload_product_occations) && !empty($upload_product_occations)? $arr_final_occations[$key]       = $upload_product_occations   : NULL;
...
// ----------------------------upload_product----------------------------
			...
			foreach ($arr_final_product as $key => $product_value) 
			{
				...
				if($obj_insert_product)
				{
					...
					$product_occations = isset($arr_final_occations[$key])? $arr_final_occations[$key] :'';
					
					foreach ($product_occations as $product_occation) 
					{
	...
	public function upload_images(Request $request)
	{
		...
		if($files!=null)
		{
			foreach ($files as $key => $file) 
			{
				...
				{
					$data['status'] = 'error';
					$data['msg'] = 'Invalid image name '.$product_image;
		...
		\Excel::create('INSTRUCTIONS', function($excel) use(
			...
			$excel->sheet('Sample Product', function($sheet)
			{
				...
				$sheet->row($i,array(
					...
					'Dilivary Date(e.g 4-5 Days)',
					'Occation1 Name, Occations2 Name (Multiple)',
