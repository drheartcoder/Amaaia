...
class MyOrdersController extends Controller
{
                                ...
                                )
    {
        ...
        $this->module_url_path               = $this->admin_url_path."/my_orders";
        $this->module_title                  = "My Orders";
        $this->module_view_folder            = "admin.my_orders";
    ...
    */
    ...
    public function new_load_data(Request $request)
    {
        ...
        $obj_orders = $obj_orders->where(function($query){
                                    $query->where('status', '1');
                                    $query->orWhere('status', '2');
                                    $query->orWhere('status', '3');
                                    $query->orWhere('status', '4');
                                 })
                                 ->with(['order_products' => function($query) use ($admin_id){
                                    $query->select('id', 'order_id', 'product_id','product_supplier_id', 'product_quantity');
                                    $query->where('product_supplier_id', $admin_id);
                                    $query->selectRaw('sum(product_price) as total_amount');
                                    $query->groupBy('order_id');
                                 }])
                                 ->whereHas('order_products.product_details', function($query) use ($admin_id){
                                    $query->select('id', 'added_by_user_type', 'added_by_user_id');
                                    $query->where('added_by_user_type', '1');
                                 })
                                 ->whereHas('order_products', function($query) use($admin_id) {
                                    $query->where('product_supplier_id', $admin_id);
                                 })
                                 ->orderBy('id','DESC');
    ...
    */
    ...
    public function return_load_data(Request $request)
    {
        ...
        $obj_return_request = $obj_return_request->select(['id','order_id','user_id','order_product_id','reason','delivery_method','refund_payment_method','status','created_at'])
                                                ...
                                                ->whereHas('order_product_details.product_details', function($query) use ($admin_id){
                                                    $query->select('id', 'added_by_user_type', 'added_by_user_id');
                                                    $query->where('added_by_user_type', '1');
                                                 })
                                                 ->whereHas('order_product_details', function($query) use($admin_id) {
                                                    $query->where('product_supplier_id', $admin_id);
                                                 })
                                                ->orderBy('created_at','DESC');
        //dd($obj_return_request->get()->toArray());
