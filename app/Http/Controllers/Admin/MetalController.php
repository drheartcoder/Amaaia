...
class MetalController extends Controller
{
	...
	public function __construct(MetalsModel $metals_model)
	{
		$this->arr_data           = [];
		$this->MetalsModel        = $metals_model;
		$this->admin_panel_slug   = config('app.project.admin_panel_slug');
		$this->admin_url_path     = url(config('app.project.admin_panel_slug'));
		$this->module_url_path    = $this->admin_url_path."/metals";
		$this->module_title       = "Metals";
		$this->module_view_folder = "admin.metals";
		$this->module_icon        = "icon-cube4";
		$this->BaseModel          = $metals_model;
	}
	...
	public function store(Request $request)
	{
		...
		$metal_name    = $request->input('metal_name', null);
		$slug          = str_slug($metal_name);

		$dose_exist    = $this->BaseModel->where(['metal_name'=> $metal_name])->count();
	...
	public function edit($enc_id)
	{
		...
		$obj_metal = $this->BaseModel->where('id',$id)->select('metal_name')->first();
		...
		$this->arr_data['arr_metal']           = $arr_metal;
	...
	public function update(Request $request, $enc_id)
	{
		...
		$dose_exist = $this->BaseModel->where(['metal_name'=> $metal_name])->where('id', '!=', $id)->count();
	...
	public function load_data(Request $request)
	{
		...
		$obj_metals = $this->BaseModel;

		if(isset($arr_search_column['q_metalname']) && $arr_search_column['q_metalname']!="")
		{
			$obj_metals = $obj_metals->where('metal_name', 'LIKE', "%".$arr_search_column['q_metalname']."%");	
		}

		if(isset($arr_search_column['q_metaldate']) && $arr_search_column['q_metaldate']!="")
		{
			$obj_metals = $obj_metals->where('created_at', 'LIKE', "%".$arr_search_column['q_metaldate']."%");	
		}
		$obj_metals = $obj_metals->select(['id', 'metal_name', 'metal_color', 'metal_quality', 'status', 'created_at']);

		if($obj_metals)
		{
			$json_result  = DataTables::of($obj_metals)->make(true);
			...
			foreach ($build_result->data as $key => $data) 
			{
				...
				if(isset($build_result->data) && sizeof($build_result->data)>0)
				{
					...
					$metal_name    = isset($data->metal_name)? $data->metal_name :'';
					$status        = isset($data->status)? $data->status :'';
					$created_at    = isset($data->created_at)? get_formated_created_date($data->created_at) :'';

					$build_result->data[$key]->id                  = $id;				
					$build_result->data[$key]->metal_name          = $metal_name;				
