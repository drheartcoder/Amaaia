...
class MetalQualityController extends Controller
{
    
    use MultiActionTrait;	
	public function __construct(MetalQualityModel $quality_model)
	{
		$this->arr_data           = [];
		$this->MetalQualityModel        = $quality_model;
		$this->BaseModel        = $quality_model;
		$this->admin_panel_slug   = config('app.project.admin_panel_slug');
		$this->admin_url_path     = url(config('app.project.admin_panel_slug'));
		$this->module_url_path    = $this->admin_url_path."/metal_qualities";
		$this->module_title       = "Qualities";
		$this->module_view_folder = "admin.metal_qualities";
	...
	public function store(Request $request)
	{
		...
		$arr_rules['quality_name']  = "required";
		...
		$quality_name = $request->input('quality_name', null);
		$slug       = str_slug($quality_name);

		$dose_exist = $this->BaseModel->where('quality_name', '=', $quality_name)->count();
	...
	public function edit($enc_id)
	{
		...
		$obj_quality = $this->BaseModel->where('id',$id)->select('quality_name')->first();
	...
	public function update(Request $request, $enc_id)
	{
		...
		if($obj_quality->quality_name == $quality_name)
		{
		...
		$dose_exist = $this->BaseModel->where('quality_name', '=', $quality_name)->where('id', '!=', $id)->count();
	...
	public function load_data(Request $request)
	{
		...
		{
			$obj_qualities = $obj_qualities->where('quality_name', 'LIKE', "%".$arr_search_column['q_qualityname']."%");	
		}
		...
		$obj_qualities = $obj_qualities->select(['id', 'quality_name', 'status', 'created_at']);
		...
		if($obj_qualities)
		{
			...
			foreach ($build_result->data as $key => $data) 
			{
				...
				if(isset($build_result->data) && sizeof($build_result->data)>0)
				{
					...
					$quality_name = isset($data->quality_name)? $data->quality_name :'';
					$status     = isset($data->status)? $data->status :'';
					$created_at = isset($data->created_at)? get_formated_created_date($data->created_at) :'';

					$build_result->data[$key]->id          = $id;				
					$build_result->data[$key]->quality_name          = $quality_name;				
