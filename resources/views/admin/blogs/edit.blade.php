...
<div class="content">
	...
	<div class="panel panel-flat">
		...
		<div class="panel-body">
			...
			<form class="form-horizontal" id="frm_edit_blog" name="frm_edit_blog" action="{{$form_url_path}}/update/{{$id}}" method="post" enctype="multipart/form-data">
				{{csrf_field()}}
				<fieldset class="content-group">
					<div class="form-group" id="category_block">
						<label class="control-label col-lg-2" for="site_name">Blog Category<i class="red">*</i></label>
						<div class="col-lg-5">
							<select class="form-control" id="category_id" name="category_id" data-rule-required="true">

								<option value="">Select Category</option>
								@if(isset($data))
								@foreach($data as $category)
								<option value="{{base64_encode($category['id'])}}" @if($blog['blog_category_id']==$category['id']) selected="selected" @endif>{{$category['category_name']}}</option>
								@endforeach
								@endif
							</select>
							<span class="error_category error">{{ $errors->first('category_id') }} </span>
						</div>
					</div>

					<div class="form-group">
						<label class="control-label col-lg-2" for="category_id">Blog Title<i class="red">*</i></label>
						<div class="col-lg-5">
							<input type="text" name="title" value="{{$blog['title'] or ''}}" id="title" class="form-control" placeholder="Blog Name" data-rule-required="true"  data-rule-maxlength="60" value="">
					...
					<div class="form-group">
						...
						<div class="col-lg-6">
							<textarea class="form-control" data-rule-required='true' placeholder="Blog Description" name="description" id="description"  rows="15">{{$blog['description'] or ''}}</textarea>
						</div>
					</div>

					<div class="form-group">
						<label class="control-label col-lg-2" for="description">Blog Image<i class="red">*</i></label>
						<div class="col-lg-5">
									...
									<div class="add_pht upload-pic loc_add_pht" id="div_blank" onclick="return add_image(this)" style="height: 150px;width: 150px; float: left;"> 
										...
										<div class="show_photos" id="show_photos" style="width: auto; display: initial;float: none;">
											<div class='photo_view2' style='width:150px;height:150px;position:relative;display: inline-block;'>
												...
												<img src="{{$blog_image_path.$blog['blog_image']}}" class='add_pht' id='add_pht upload-pic' style='float: left; padding: 0px ! important; margin:0' width='120' height='120'>
												<div class='overlay2'>
										...
										<div id="div_hidden_photo_list" class="div_hidden_photo_list">
											<input type="file" name="blog_image" id="blog_image" class="blog_image" style="display:none" @if($blog['blog_image']=='') data-rule-required="true" @endif />
										</div>
									</div>
							...
							<div class="col-lg-10 col-lg-offset-2">
								<i class="note col-lg-12 red">Only jpg, png, gif, jpeg type images are allowed<br>Image size should be upto 2 MB only.</i>
							</div>
						</div>

						<div class="form-group text-center">
							<div class="col-lg-7">
								<button type="submit" class="btn btn-primary" id="btn_update_blog">Update</button>
			...
			<script>
				...
				$(document).ready(function(){
					...
					$('#btn_update_blog').click(function(){
				      		tinyMCE.triggerSave();
				    });
