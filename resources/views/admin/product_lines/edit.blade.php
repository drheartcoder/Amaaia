...
<div class="content">
	...
	<div class="panel panel-flat">
		...
		<div class="panel-body">
			<form class="form-horizontal" id="frm_add_product_line" name="frm_add_product_line" action="{{$module_url_path}}/update/{{$id}}" method="post" enctype="multipart/form-data">
				{{csrf_field()}}
				<fieldset class="content-group">
					<div class="form-group">
						...
						<div class="col-lg-5">
							<select class="form-control" id="product_type" name="product_type" data-rule-required="true" data-msg-required="Please select the product type." onchange="load_categories()">
								...
								<option value="1" {{ $arr_product_line['product_type'] == 1 ? 'selected' : ''}}>Classic</option>
								<option value="2" {{ $arr_product_line['product_type'] == 2 ? 'selected' : ''}}>Luxure</option>
							</select>
					...
					<div class="form-group" id="category_block">
						...
						<div class="col-lg-5">
							<select class="form-control" id="category_name" name="category_name" data-rule-required="true" data-msg-required="Please select the product type." onchange="load_subcategories()">
								...
								@if(isset($arr_categories) && is_array($arr_categories) && sizeof($arr_categories)>0)
								@foreach($arr_categories as $category)
								<option value="{{$category['id'] or ''}}" @if($category['id']==$arr_product_line['category_id']) selected="selected" @endif>{{$category['category_name'] or ''}}</option>
								@endforeach
								@endif
							</select>
					...
					<div class="form-group" id="category_block">
						...
						<div class="col-lg-5">
							<select class="form-control" id="subcategory_name" name="subcategory_name" data-rule-required="true" data-msg-required="Please select the subcategory.">
								...
								@if(isset($arr_subcategories) && is_array($arr_subcategories) && sizeof($arr_subcategories)>0)
								@foreach($arr_subcategories as $subcategory)
								<option value="{{$subcategory['id'] or ''}}" @if($subcategory['id']==$arr_product_line['sub_category_id']) selected="selected" @endif>{{$subcategory['subcategory_name'] or ''}}</option>
								@endforeach
								@endif
							</select>
					...
					<div class="form-group">
						...
						<div class="col-lg-5">
							<input type="text" name="product_line_name" id="product_line_name" class="form-control" placeholder="Product Line" data-rule-required="true"  data-rule-maxlength="60" value="{{$arr_product_line['product_line_name'] or ''}}">
						...
						<div class="col-sm-9 col-lg-10 controls">
							<div class="fileupload fileupload-new" data-provides="fileupload">
								...
								<div class="fileupload-new img-thumbnail" style="width: 200px; height: 150px;">
									@if(isset($arr_product_line['image']) && !empty($arr_product_line['image']) && File::exists($product_line_image_base_path.$arr_product_line['image']))
									<img src="{{$product_line_image_public_path.$arr_product_line['image']}}"  style="max-width: 100%; max-height: 100%; line-height: 20px;" class="fileupload-preview">
									@php 
									$prev_image_url = $product_line_image_public_path.$arr_product_line['image']; 
