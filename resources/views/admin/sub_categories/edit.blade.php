...
<!-- BEGIN Main Content -->
				...
				<div class="content">
						...
						<div class="panel-body">
							...
							<form class="form-horizontal" id="frm_add_sub_category" name="frm_add_sub_category" action="{{$module_url_path}}/update/{{base64_encode( $arr_sub_category['id'])}}" method="post" enctype="multipart/form-data">
								{{csrf_field()}}
								<fieldset class="content-group">	
											...
											<div class="fileupload fileupload-new" data-provides="fileupload">
												...
												<div>
													<span class="btn btn-default btn-file" style="height:32px;">
														...
														value="{{ $arr_sub_category['image']  or ''}}"/>
									...
									<div class="form-group">
										...
										<div class="col-lg-5">
											<select class="form-control" id="product_type" name="product_type" data-rule-required="true" data-msg-required="Please select the product type." onchange="get_category(this,'change')">
													...
													<option value="1" {{ isset($arr_sub_category['product_type']) && $arr_sub_category['product_type'] == 1 ? 'selected' : ''}}>Classic</option>
													<option value="2" {{ isset($arr_sub_category['product_type']) && $arr_sub_category['product_type'] == 2 ? 'selected' : ''}}>Luxure</option>
											</select>
									...
									<div class="form-group">
										...
										<div class="col-lg-5">
											<input type="text" name="sub_category_name" id="sub_category_name" class="form-control" placeholder="Sub Category Name" data-rule-required="true"  data-rule-maxlength="60" value="{{$arr_sub_category['subcategory_name'] or ''}}">
											<span class="error error_sub_category">{{ $errors->first('sub_category_name') }} </span>
										</div>
									</div>
									...
									<div class="form-group">
										...
										<div class="col-lg-5">
											<input type="text" name="market_orientation_markup" id="market_orientation_markup" class="form-control" placeholder="Market Orientation Markup" data-rule-required="true"  data-rule-maxlength="5" data-rule-max="99" data-rule-pattern="\d+(\.\d{1,2})?" data-msg-pattern="Please enter valid percentage." data-msg-maxlength="Please enter no more than 5 digits." value="{{$arr_sub_category['market_orientation_markup'] or ''}}">
											<span class="error error_market_orientation_markup">{{ $errors->first('market_orientation_markup') }} </span>
										</div>
									</div>
									...
									<div class="form-group">
										...
										<div class="col-lg-5">
											<textarea name="description" id="description" class="form-control" placeholder="Description" data-rule-required="true"  data-rule-maxlength="555" rows="5">{{$arr_sub_category['description'] or ''}}</textarea>
...
<script>
    ...
    $(document).ready(function(){
    	...
    	@if(isset($arr_sub_category['product_type']) && !empty($arr_sub_category['product_type']))
    	var product_type =   '{{$arr_sub_category['product_type'] or ''}}';
			...
			success:function(data){
				if(data)
				{
					...
					$.each(data,function(i, obj){
						...
						if(action == 'page_load')
				    	{
				    		 category_id =   '{{$arr_sub_category['category_id'] or ''}}';
