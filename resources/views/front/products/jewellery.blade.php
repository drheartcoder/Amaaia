...
$sub_category_description    = isset($sub_category['description']) ? $sub_category['description'] : '';
...
<div class="bradcrum-inner">
 ...
 <div class="pul-right-sublink">
    <a href="{{ url('/') }}">Home</a> / <a>{{ $category_name }}</a> / <span>{{ $sub_category_name }}</span>
</div>
...
<div class="min-hieght-class">
   <div class="ring-product-bg-pattern">
       ...
       <p>{{ str_limit($sub_category_description, 300) }}</p>
   </div>   

   <div class="container-fluid">

    @if(isset($productline_data) && !empty($productline_data))
    <div class="col-md-12"><div class="title-product-likes tp-mrgin-bx">By Product Line Like</div></div>
    <div class="box-center">

        @foreach($productline_data as $pl_data)
        
        @php
        $product_line_id    = isset($pl_data['id']) ? $pl_data['id'] : '';
        $product_line_name  = isset($pl_data['product_line_name']) ? $pl_data['product_line_name'] : '';
        $product_line_slug  = isset($pl_data['slug']) ? $pl_data['slug'] : '';
        @endphp
        <div class="lisitng-box-twenty">
         <div class="lisitng-box-twentyinner">
            <a @if($active_product_line_slug == $product_line_slug) class="active" @endif href="{{ url('/') }}/{{ $category_slug }}/{{ $sub_category_slug }}/{{ $product_line_slug }}">
                <!-- <span class="img-icon-list solitare-icns"></span> -->
                <span class="name-box-lisitng">{{ $product_line_name }}</span>
            </a>
        </div>
    </div>
    @endforeach
            ...
            <div class="sorting-block">
                ...
                <div class="select-style select2">
                    <select class="frm-select" id="cmb_sort_by" name="sort_by">
                        ...
                        <option value="low_to_high" @if(Request::input('sort_by') == 'low_to_high') selected @endif>Price Low to High</option>
                        <option value="high_to_low" @if(Request::input('sort_by') == 'high_to_low') selected @endif>Price High to Low</option>
                    </select>
                </div>
            </div>
        </div>
